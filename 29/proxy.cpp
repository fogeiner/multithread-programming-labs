...
class ClientState {
	public:
		...
		virtual void init(Client *c){
			Logger::info("ClientState unhandled init()");
		}
	protected:
...
class ClientBadRequest: public ClientState {
	ClientBadRequest(){}
	public:
	static ClientState *instance(){
		static ClientBadRequest cbr;
		return &cbr;
	}
...
bool ClientBadRequest::writable(const Client *c){
	return c->_b->size() > 0;
}
void ClientBadRequest::handle_write(Client *c) {
	try{
...
void ClientBadRequest::init(Client *c){
	// clear buffer
	c->_b->drop_first(c->_b->size());
	...
	try {
		...
		if (p != NULL) {
			...
			try {
				...
				ClientState *s = ClientBadRequest::instance();
				s->init(c);
				c->change_state(s);
			}
